...
function createOccurrence(event) {
  ...
  // Melhoria: Verificação de permissão (Princípio do Menor Privilégio)
  if (session && session.role === "ALUNO") {
    alert("Erro: Alunos não possuem permissão para criar ocorrências.");
    writeLog("ACESSO_NEGADO", "Tentativa de criação de ocorrência por perfil ALUNO.");
    return;
  }

  const occurrence = {
    id: `OC-${Math.floor(Math.random() * 9000) + 1000}`,
    studentName: sanitizeHTML(document.querySelector("#studentName").value),
    studentId: sanitizeHTML(document.querySelector("#studentId").value),
    studentCpf: sanitizeHTML(document.querySelector("#studentCpf").value),
    studentEmail: sanitizeHTML(document.querySelector("#studentEmail").value),
    studentPhone: sanitizeHTML(document.querySelector("#studentPhone").value),
    category: document.querySelector("#category").value,
    priority: document.querySelector("#priority").value,
    description: sanitizeHTML(document.querySelector("#description").value),
    internalNote: sanitizeHTML(document.querySelector("#internalNote").value),
...
function deleteOccurrence(id) {
  ...
  // Melhoria: Restrição de exclusão apenas para ADMIN
  if (!session || session.role !== "ADMIN") {
    alert("Erro: Apenas administradores podem excluir ocorrências.");
    writeLog("ACESSO_NEGADO", `Tentativa de exclusão da ocorrência ${id} por ${session ? session.role : 'anônimo'}.`);
    return;
  }
...
function render() {
  ...
  const filtered = occurrences.filter((item) => {
    // Melhoria: Alunos só veem suas próprias ocorrências (simulado por nome para fins didáticos)
    if (session && session.role === "ALUNO") {
      return item.studentName === session.name && JSON.stringify(item).toLowerCase().includes(term);
    }
  ...
  criticalOccurrences.textContent = filtered.filter((item) => item.priority === "Crítica").length;
  lastUpdate.textContent = `Atualizado em ${new Date().toLocaleTimeString("pt-BR")}`;

  occurrencesTable.innerHTML = filtered.map((item) => {
    // Melhoria: Mascaramento de dados sensíveis para perfis não-admin
    const displayCpf = (session && session.role === "ADMIN") ? item.studentCpf : "***.***.***-**";
    const displayInternalNote = (session && (session.role === "ADMIN" || session.role === "PROFESSOR")) ? item.internalNote : "<em>Acesso restrito</em>";
    ...
    <tr>
      ...
      <td>
        ...
        <strong>Obs. interna:</strong> ${displayInternalNote}
      </td>
      <td>
